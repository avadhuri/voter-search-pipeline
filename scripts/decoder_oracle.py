...
def do_part(ac, member, raw_dir, out, dpi, pages_per_part, project, dry_run, seed):
    ...
    for page_no in pages:
        ...
        if resp is None:
            resp = annotate(rendered[page_no], project)
            _store(cache, resp)
        words, pw, ph = vision_words(resp)
...
def print_report(rep: dict) -> None:
    ...
    print(f"                rendered locally at {s['dpi']} dpi with the "
          f"embedded font; Vision read pixels, never a text layer")
    if not rep["cells"]:
...
def main(argv=None):
    ...
    picks = choose_parts(args.raw_dir, n_parts, args.seed)
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        futures = {
            pool.submit(do_part, ac, member, args.raw_dir, args.out, args.dpi,
                        args.pages_per_part, args.project, dry, args.seed): ac
