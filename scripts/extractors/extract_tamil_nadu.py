"""
Extract Tamil Nadu old SIR (2005) voter roll PDFs to CSV.

PDF uses Tamil + CID encoding. Text extracted as-is using word-position
column detection. Column number markers (1)(2)...(8) or 1 2...8 used
for alignment.

Usage:
    python scripts/extractors/extract_tamil_nadu.py                    # all downloaded ACs
    python scripts/extractors/extract_tamil_nadu.py --ac 1,2,3         # specific ACs
    python scripts/extractors/extract_tamil_nadu.py --limit 5          # first 5 ACs
    python scripts/extractors/extract_tamil_nadu.py --combined         # single CSV for state
"""
import argparse
import csv
import io
...
def main():
    ap = argparse.ArgumentParser(description="Extract Tamil Nadu SIR PDFs to CSV")
    ...
    ap.add_argument("--out-dir", default="output/csv/tamil_nadu")
