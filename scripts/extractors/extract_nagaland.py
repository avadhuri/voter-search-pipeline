"""
Extract Nagaland old SIR voter roll PDFs to CSV.
...
Usage:
    python scripts/extract_nagaland.py                    # all downloaded ACs
    python scripts/extract_nagaland.py --ac 1,2,3         # specific ACs
    python scripts/extract_nagaland.py --limit 5          # first 5 ACs
    python scripts/extract_nagaland.py --combined         # single CSV for state
"""
import argparse
import csv
import io
...
def _find_col_row_top(words, col_centres):
    ...
    Requires 3+ column markers at the same y so lone data digits (serial
    numbers, house numbers) aren't mistaken for column headers.
    """
    candidates = []
    for w in words:
        txt = w["text"]
...
def _postprocess(csv_path):
    ...
    Nagaland-specific fixes:
    1. Relation column has OCR misreads (N, B, S, L, G, D, I, 1, -)
       → clear to empty.
    """
...
def main():
    ap = argparse.ArgumentParser(description="Extract Nagaland SIR PDFs to CSV")
    ...
    ap.add_argument("--out-dir", default="output/csv/nagaland")
