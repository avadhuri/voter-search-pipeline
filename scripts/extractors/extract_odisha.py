"""
Extract Odisha old SIR (2002) voter roll PDFs to CSV.

PDF uses Odia Unicode (OROT-Mukta). Text extracted as-is using word-position
column detection. Column number markers (1)(2)...(8) or 1 2...8 used for alignment.

Usage:
    python scripts/extractors/extract_odisha.py                    # all downloaded ACs
    python scripts/extractors/extract_odisha.py --ac 1,2,3         # specific ACs
    python scripts/extractors/extract_odisha.py --limit 5          # first 5 ACs
    python scripts/extractors/extract_odisha.py --combined         # single CSV for state
"""
import argparse
import csv
import io
...
N_COLS = 8
NARROW_COLS = {4, 6, 7}
# Fallback column centres when (1)(2)...(8) markers are absent or undetectable.
# Odisha PDFs have a two-line layout per voter: line 1 has serial(@34),
# section+house(@82,@134), relation_code(@276), sex(@414), age(@446),
# epic(@474); line 2 has elector_name(@162) and relation_name(@309).
# Centres are chosen so section+house_no both land in col 2 (centre=108,
# captures x=82 and x=134) while elector_name text (x>=162) lands in
# col 3 (centre=200).
FALLBACK_CENTRES = {1: 34, 2: 108, 3: 200, 4: 276, 5: 340, 6: 414, 7: 446, 8: 474}
ROW_TOL = 5.0
CSV_HEADERS = [
...
def _extract_page(page, fallback_centres=None):
    ...
    for rw in rows:
        ...
        if serial and re.fullmatch(r"\d+", serial):
            # Primary row with serial number — start a new record
            _fix_row(cells)
            records.append(cells)
        elif records:
            # Continuation row (no serial) — merge non-empty cells into
            # the previous record.  Odisha PDFs put the elector name and
            # relation name on a second line below the serial row.
            prev = records[-1]
...
def main():
    ap = argparse.ArgumentParser(description="Extract Odisha SIR PDFs to CSV")
    ...
    ap.add_argument("--out-dir", default="output/csv/odisha")
