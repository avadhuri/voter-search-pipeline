...
Names are in Devanagari Unicode (CDAC_GISTSurekh font) - extractable but
with some character mapping issues (half-forms may render differently).

Usage:
    python scripts/extractors/extract_madhya_pradesh.py
        ...
        with zipfile.ZipFile(os.path.join(raw_dir, zf)) as z:
            ...
            for pf in sorted(n for n in z.namelist() if n.endswith(".pdf")):
                ...
                for row in extract_pdf_table(z.read(pf)):
                    # row: [serial, house, name, relation, rel_name, gender, age, epic]
                    r = (row + [""] * 8)[:8]
                    ac_rows.append([
