"""
Extract Mizoram old SIR voter roll PDFs to CSV.

PDF layout: 8 columns
  1. Serial No  2. House No  3. Elector Name  4. Relation (F/H/M/O)
  5. Relation Name  6. Sex (M/F)  7. Age  8. EPIC No (full alphanumeric)

Usage:
    python scripts/extract_mizoram.py                    # all downloaded ACs
    python scripts/extract_mizoram.py --ac 1,2,3         # specific ACs
    python scripts/extract_mizoram.py --limit 5          # first 5 ACs
    python scripts/extract_mizoram.py --combined         # single CSV for state
"""
import argparse
import csv
import io
...
def _fix_row(row):
    ...
    Mizoram names often start with initials (C., K., A., BH.) that land
    in the house_no column boundary.
    """
    HOUSE = 2
    NAME = 3
    REL = 4
...
VALID_SEX = {"F", "M", ""}
VALID_RELATION = {"F", "H", "M", "O", "D", "G", "K", "N", "R", "S", "W", "", "0"}


def _postprocess(csv_path):
    """Validate sex and relation columns in an already-extracted CSV.

    Mizoram-specific valid values:
      sex: F, M
      relation: F, H, M, O, D, G, K, N, R, S, W, 0
...
def main():
    ap = argparse.ArgumentParser(description="Extract Mizoram SIR PDFs to CSV")
    ...
    ap.add_argument("--out-dir", default="output/csv/mizoram")
