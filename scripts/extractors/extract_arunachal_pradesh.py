"""
Extract Arunachal Pradesh old SIR voter roll PDFs to CSV.
...
Usage:
    python scripts/extract_arunachal_pradesh.py                    # all downloaded ACs
    python scripts/extract_arunachal_pradesh.py --ac 1,2,3         # specific ACs
    python scripts/extract_arunachal_pradesh.py --limit 5          # first 5 ACs
    python scripts/extract_arunachal_pradesh.py --combined         # single CSV for state
"""
import argparse
import csv
import io
...
def _find_col_row_top(words, col_centres):
    ...
    Requires at least 3 column markers clustered at the same y, so a lone
    data digit like serial '1' doesn't get mistaken for a header.
    """
    candidates = []
    for w in words:
        txt = w["text"]
...
def main():
    ap = argparse.ArgumentParser(description="Extract Arunachal Pradesh SIR PDFs to CSV")
