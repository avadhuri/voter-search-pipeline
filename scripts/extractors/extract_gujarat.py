"""
Extract Gujarat old SIR (2002) voter roll PDFs to CSV.

PDF uses Gujarati CID encoded. Text extracted as-is using word-position
column detection. Column number markers (1)(2)...(8) or 1 2...8 used
for alignment.

Usage:
    python scripts/extractors/extract_gujarat.py                    # all downloaded ACs
    python scripts/extractors/extract_gujarat.py --ac 1,2,3         # specific ACs
    python scripts/extractors/extract_gujarat.py --limit 5          # first 5 ACs
    python scripts/extractors/extract_gujarat.py --combined         # single CSV for state
"""
import argparse
import csv
import io
...
def main():
    ap = argparse.ArgumentParser(description="Extract Gujarat SIR PDFs to CSV")
    ...
    ap.add_argument("--out-dir", default="output/csv/gujarat")
