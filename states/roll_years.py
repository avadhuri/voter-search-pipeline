...
`state_id` onto it needs a lookup -- `states/eci_codes.py`, shared with
`source_urls.py`, which needs the identical mapping to find a state's
per-part workbook. That table is the single place a new state declares
which ECI code it is.
...
def resolve_roll_year(state_id, default=DEFAULT_ROLL_YEAR):
    ...
    Falls back to `default` for a state with no `eci_codes.STATE_CODES`
    entry, or whose ECI code isn't in the metadata file -- see the module
    docstring for why that's a fallback rather than an error.
    """
